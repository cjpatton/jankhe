...
use crate::{
    PubEnc,
    poly::{PolyRing, slow_poly_mul_bigint},
};
...
impl SomewahtHomomorphic for Bfv {
    ...
    fn relin_key_gen(&self, _sk: &Self::SecretKey) -> Self::RelinKey {
        todo!()
    }

    fn relin(&self, _rk: &Self::RelinKey, _c: &mut BfvCiphertext) {
        todo!()
    }
        ...
        */
        ...
        for (i, x) in c1.iter().map(|x| x.to_bigints()).enumerate() {
            for (j, y) in c2.iter().map(|y| y.to_bigints()).enumerate() {
                ...
                for (o, z) in out[i + j].iter_mut().zip(z.into_iter()) {
                    *o += z;
                }
            }
        }
...
mod tests {
    ...
    #[ignore = "can't seem to get this to work"]
    #[test]
